...
): StateCreator<StoreType<StoresDataType[S]>> => (set, get, api): StoreType<StoresDataType[S]> =>
  config(
    args => {
      ...
      if (global.enableStoreLogging) {
        console.group(`%c🗂 ${store.toLocaleUpperCase()} STORE UPDATED`, 'font-weight:bold')
        console.log('%cprevState', 'font-weight:bold; color: #9E9E9E', prevState)
